...
3. Spring Security 集成HTTP摘要认证
   ...
   1. 开发授权（Open Authorization）

      > 是一种资源提供商用于授权第三方应用代表资源提供者获取有限访问权限的授权机制。由于在整个授权过程中，第三方应用都无须触及用户的密码就可以取得部分资源的使用权限，所以OAuth是安全开放的

      > 解决了在用户不提供密码给第三方应用的情况下，让第三方应用有权获取用户数据以及基本信息的难题

2. OAuth 4个重要的角色
   - Resource Owner    资源所有者，通常指用户，例如每一个QQ用户
   - Resource Server    资源服务器，指存放用户受保护资源的服务器，通常需要通过 Access Token（访问令牌）才能进行访问。例如，存储QQ用户基本信息的服务器，充当的便是资源服务器的角色
...
3. OAuth 运行流程
   ...
   ![OAuth运行流程](./OAuth运行流程.jpg)

- （A） 客户端要求用户提供授权许可
- （B） 用户同意向客户端提供授权许可
- （C） 客户端携带用户提供的授权许可向授权服务器申请资源服务器的访问令牌
- （D） 授权服务器验证客户端以及其携带的授权许可，确认有效后发放访问令牌
- （E） 客户端使用访问令牌向资源服务器申请资源
- （F） 资源服务器验证访问令牌，确认无误后向客户端提供资源

6. Authorization Grant 的四种授权模式

   - 授权码模式（Authorization Code）：授权码模式是功能最完整、流程最严密的授权模式，它将用户引导到授权服务器进行身份验证，授权服务器将发放的访问令牌传递给客户端。例如，前面介绍的QQ登录方法，就是由CSDN网站引导到QQ授权服务器进行身份验证的 [QQ授权登录开发指南]([https://wiki.connect.qq.com/%E5%87%86%E5%A4%87%E5%B7%A5%E4%BD%9C_oauth2-0](https://wiki.connect.qq.com/准备工作_oauth2-0))  

     ![QQ授权码流程](OAuth授权码流程.png)

   - 隐式授权模式（Implicit）：该模式客户端一般是指浏览器。访问令牌通过重定向的方式传递到用户浏览器中，再通过浏览器的 JavaScript 代码来获取访问令牌。由于访问令牌直接暴露在浏览器端，所以隐式授权模式可能会导致访问令牌被黑客获取，仅适用于需要临时访问的场景。例如，QQ的OAuth针对客户端用户的便是隐式授权模式。与授权码模式相比，授权码模式是携带一个认证码，由客户端（第三方应用后端程序）通过认证码申请访问令牌的；而隐式授权模式直接将访问令牌作为URL的散列部分传递给浏览器

   - 密码授权模式（Password Credentials）：就是客户端直接携带用户的密码向授权服务器申请令牌。这种登录操作不再像前两种授权模式一样跳转到授权服务器进行，而是客户端提供专用。如果用户信任客户端（通常为信誉度高的著名公司），用户便可以直接提供密码，客户端在不存储用户密码的前提下完成令牌的申请

   - 客户端授权模式（Client Credentials）：实际上并不属于OAuth的范畴，它的关注点不再是用户的私有信息或数据，而是一些由资源服务器持有但并非完全公开的数据，如微信的公众平台授权等。该模式通常由客户端提前向授权服务器申请应用公钥、秘钥，并通过这些关键信息向授权服务器申请访问令牌，从而得到资源服务器提供的资源
